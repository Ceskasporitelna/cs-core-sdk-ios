...
public class RSAUtils: NSObject {
    ...
    static public func encryptWithRSAKey(_ data: Data, rsaKeyRef: SecKey, padding: SecPadding) -> Data? {
        ...
        while (idx < decryptedDataAsArray.count ) {
            ...
            if ( status != noErr ) {
                clog(CoreSDK.ModuleName, activityName: CoreSDKActivities.DataEncryption.rawValue, fileName: #file, functionName: #function, lineNumber: #line, logLevel: LogLevel.error, format: "Error while ecrypting: %i", status );
                return nil
            }
    ...
    static fileprivate func stripPublicKeyHeader(_ pubkey: Data) -> Data?
    {
        ...
        let range: Range<Data.Index> = idx..<keyAsArray.count
        return pubkey.subdata(in: range)
    }
    ...
    static fileprivate func stripPrivateKeyHeader(_ privkey: Data) -> Data? {
        ...
        let range: Range<Data.Index> = idx..<idx+len
        return privkey.subdata(in: range)
    }
    ...
    static public func encryptWithRSAPrivateKey(_ data: Data, privkeyBase64: String, keychainTag: String) -> Data? {
        ...
        return encryptWithRSAKey(data, rsaKeyRef: keyRef!, padding: SecPadding.OAEP) // Padding changed
    }
